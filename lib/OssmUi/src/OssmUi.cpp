...
static void OssmUiOverlaySpeed(OLEDDisplay* display, OLEDDisplayUiState* state)
{
    ...
    display->drawString(0, 0, l_titel);
    display->drawString(80, 0, r_titel);
...
static void OssmUiFrameKMlogo(OLEDDisplay* display, OLEDDisplayUiState* state, int16_t x, int16_t y)
{
    ...
    if (logo_on == true){
    display->drawXbm(x + 44, y + 2, 40, 40, km_logo);
    }
...
void OssmUi::UpdateStateL(const int speed_percentage)
{
    s_speed_percentage = speed_percentage;
}

void OssmUi::UpdateStateR(const int encoder_position)
{
    s_encoder_position = encoder_position;
}
void OssmUi::UpdateTitelL( String l_titel_in)
{
    l_titel = l_titel_in;
    m_ui.update();
}

void OssmUi::UpdateTitelR( String r_titel_in)
{
    r_titel = r_titel_in;
    m_ui.update();
}
...
void OssmUi::ShowMenue()
{
    m_ui.switchToFrame(2);
}
